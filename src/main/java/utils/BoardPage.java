...
public class BoardPage {
	...
	public static String pagingStr(int totalCount, int pageSize, int blockPage, 
		...
		if(pageTemp != 1) {
			...
			pagingStr += "<li class='page-item'><a class='page-link' href='"
					+ reqUrl +"?pageNum="+ (pageTemp - 1) +"'><i class='bi bi-skip-start-fill'></i></a></li>";
		...
		while(blockCount <= blockPage && pageTemp <= totalPages) {
			if(pageTemp == pageNum) {
				//pagingStr += "&nbsp;" + pageTemp + "&nbsp;";
				pagingStr += "<li class='page-item active'><a class='page-link' href='#'>" 
						+ pageTemp + "</a></li>";
			} 
			else {
		...
		if(pageTemp <= totalPages) {
			pagingStr += "<li class='page-item'><a class='page-link' href='" 
					+ reqUrl + "?pageNum=" + pageTemp + "'><i class='bi bi-skip-end-fill'></i></a></li>";
			//pagingStr += "<a href='" + reqUrl + "?pageNum=" + pageTemp + "'>[다음 블록]</a>";
			//pagingStr += "&nbsp;";
			pagingStr += "<li class='page-item'><a class='page-link' href='" 
					+ reqUrl + "?pageNum=" + totalPages + "'><i class='bi bi-skip-forward-fill'></i></a></li>"; 
